...
class FES_Profile_Form extends FES_Form {
	...
	public function set_title() {
		$title = _x( 'Profile', 'FES Form title translation', 'olam' );
	...
	public function can_render_form( $output = false, $is_admin = -2, $user_id = -2 ) {
		...
		if ( $is_admin ) {
			...
			if ( !$is_a_admin && !$is_a_vendor && !( $is_a_vendor && $user == $user_id ) ) {
				if ( $output ) {
					return sprintf( _x( 'Access Denied: You are not an admin or the %s associated with this profile.', 'FES lowercase singular setting for vendor', 'olam' ), EDD_FES()->helper->get_vendor_constant_name( $plural = false, $uppercase = false ) );
				} else {
					return false;
				}
			}
